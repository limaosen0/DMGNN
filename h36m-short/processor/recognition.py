...
class REC_Processor(Processor):
    ...
    def train(self):
        ...
        encoder_inputs, decoder_inputs, targets, downsample_idx = train_sample(normed_train_dict, 
                                                               self.arg.batch_size, 
        ...
        downsample_idx = torch.Tensor(downsample_idx).long().to(self.dev)
    ...
    def test(self, evaluation=True, iter_time=0, save_motion=False):
        ...
        for action_num, action in enumerate(actions):
            encoder_inputs, decoder_inputs, targets, downsample_idx = srnn_sample(normed_test_dict, action,
                                                                  self.arg.source_seq_len, 
                                                                  self.arg.target_seq_len, 
                                                                  len(self.dim_use))
            encoder_inputs_v = np.zeros_like(encoder_inputs)
        ...
        self.mean_MAE_select[iter_time] = np.mean((self.MAE_tensor[iter_time, :, 1]+
                                                   self.MAE_tensor[iter_time, :, 3]+
                                                   self.MAE_tensor[iter_time, :, 7]+
                                                   self.MAE_tensor[iter_time, :, 9])/4)
        self.io.print_log(str(self.mean_MAE_select[iter_time]))
