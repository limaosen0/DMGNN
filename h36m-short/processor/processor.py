...
class Processor(IO):
    ...
    def load_data(self):
        ...
        self.train_dict, self.complete_train = load_data(self.arg.data_dir, self.arg.subtrain, self.actions)
        self.test_dict, self.complete_test = load_data(self.arg.data_dir, self.arg.subtest, self.actions)
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            self.MAE_tensor = np.zeros((self.arg.iter_num//self.arg.eval_interval, 15, 13))
            self.mean_MAE_select = np.zeros(self.arg.iter_num//self.arg.eval_interval)
            ...
            self.MAE_min = self.MAE_tensor.min(axis=0)
            self.MAE_min[:,-1] = self.MAE_min[:,:10].mean(axis=-1)
            for act_num in range(15):
                print_str = str(self.MAE_min[act_num])
                self.io.print_log(print_str)
            self.averageMAE_min = self.mean_MAE_select.min(0)
            self.io.print_log(str(self.averageMAE_min))


        elif self.arg.phase == 'test':
            self.MAE_tensor = np.zeros((self.arg.iter_num//self.arg.eval_interval, 15, 12))
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--savemotion_interval', type=int, default=1000000, help='the interval for evaluating models (#iteration)')
        ...
        parser.add_argument('--data_dir', default='../data/h3.6m/dataset', help='data loader will be used')
        parser.add_argument('--subtrain', type=int, default=[1,6,7,8,9,11], help='data loader will be used')
        parser.add_argument('--subtest', type=int, default=[5], help='data loader will be used')
